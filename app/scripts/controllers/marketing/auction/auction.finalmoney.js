 ...
 */
wapApp.controller('AuctionFinalMoneyController',
    function ($scope, $cookies,$cookieStore, ConstantService, OrderService,AuctionService, $state, PayService,
	 ...
	 var uid = $cookies.get(ConstantService.LOGIN_ID_KEY);
	    ...
	    OrderService.listOrders2Clearing().success(function(data){
	        ...
	        if($scope.transaction && $scope.transaction.orders){
	            ...
	            if($cookieStore.get("address")){
	            	$scope.address=$cookieStore.get("address");
	            	$scope.transaction.addressid = $scope.address.id;
	    			$scope.transaction.city = $scope.address.city;
	    			$scope.transaction.address = $scope.address.countryName+'-'+$scope.address.provinceName+'-'+$scope.address.cityName+'-'+$scope.address.address;
	    			$scope.transaction.receiver = $scope.address.receiver;
	    			$scope.transaction.mobile = $scope.address.mobile;
	    			$scope.transaction.zipcode = $scope.address.zipcode;
	    			$scope.transaction.countryName = $scope.address.countryName;
	    			$scope.transaction.provinceName = $scope.address.provinceName;
	    			$scope.transaction.cityName = $scope.address.cityName;
	    			$scope.selectAddress($scope.address);
	            }
	            if($scope.type=="AUCTION"||$scope.type=="AUCTIONFINAL"){
	    ...
	    $scope.confirmSubmit = function(){
	        ...
	        if($scope.type=="AUCTION"){
	        	 angular.forEach($scope.purchaseVo.orders, function(order, index){
	                 order.depositmoney = $scope.finalmoney;
	             });
	        }
	 	   ...
	 	   AuctionService.detail(auctionid)
	 		.success(function(data){
	 			if(data.ok){
	 				...
	 				if($scope.type=="AUCTION"){
	 					$scope.depositmoney=$scope.auction.deposit;
